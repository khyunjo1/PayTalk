...
전문 반찬가게와 고객을 연결하는 스마트한 배달 플랫폼입니다. PWA 기술을 활용하여 모바일 앱과 같은 사용자 경험을 제공하며, 실시간 푸시 알림을 통해 주문 상태를 즉시 알려드립니다. 전화번호 기반 간편 로그인과 무통장입금/제로페이 결제 시스템을 지원합니다.

## 🚀 주요 기능

### 고객 기능
- **전화번호 로그인** - 간편한 전화번호 기반 로그인
- **일일 주문서** - 매일 업데이트되는 반찬 메뉴
- **실시간 주문** - 배달/픽업 주문 시스템
...
### 매장 관리자 기능
- **매장 정보 관리** - 매장 등록, 수정, 삭제
- **메뉴 관리** - 메뉴 CRUD, 카테고리 관리
- **일일 주문서 관리** - 날짜별 주문서 생성 및 설정
- **주문 관리** - 주문 목록, 상태 변경, 상세 조회
- **배달 지역 관리** - 지역별 배달비 설정
- **시간 설정** - 영업시간, 주문마감시간, 픽업/배달시간대

### 슈퍼 관리자 기능
- **전체 매장 관리** - 모든 매장 통합 관리
...
### 고객 사용법
1. **홈페이지** (`/`)에서 전화번호로 로그인
2. **일일 주문서** (`/menu/:storeId/daily/:date`)에서 메뉴 확인
...
3. **일일 주문서 관리** (`/admin/:storeId/daily-menu`)에서 메뉴 설정
...
- **메뉴 API** (`menuApi.ts`) - 메뉴 CRUD, 카테고리 관리
- **일일 메뉴 API** (`dailyMenuApi.ts`) - 일일 주문서 관리
...
이 프로젝트는 한국의 반찬 배달 웹 애플리케이션입니다. 사용자는 전화번호로 로그인하여 매장을 둘러보고, 메뉴를 장바구니에 담아 배달 또는 픽업 주문을 할 수 있습니다. 무통장입금과 제로페이 QR 코드를 통한 결제를 지원합니다. 매장 사장님들은 전용 페이지에서 주문을 관리할 수 있으며, 슈퍼 관리자 대시보드를 통해 운영자는 매장을 생성하고, 메뉴를 관리하며, 사장님들의 문의를 처리할 수 있습니다.

**PayTalk** - 반찬 배달의 새로운 경험을 만들어갑니다. 🍽️✨